...
def _save_expression_data(
    exprMatrixDf: pd.DataFrame, outputDir: Union[str, os.PathLike]
) -> None:
    # The visualizers expect the index to be sorted, the index name to be None and the data type to float32
    exprMatrixDf = (
        exprMatrixDf.sort_index()
        .set_index(exprMatrixDf.index.rename(None), drop=True)
        .astype(np.float32)
    )
    ...
    for column in exprMatrixDf.columns:
        save_parquet_analysis_result(
            exprMatrixDf[column].to_frame(), column, cellByIndGeneDir, index=False
        )
...
def _save_cell_metadata(
    cellMetadata: pd.DataFrame, outputDir: Union[str, os.PathLike]
) -> None:
    save_parquet_analysis_result(
        cellMetadata.index.to_frame(name="cell_indices"),
        "cells_indices",
        outputDir,
        index=False,
    )
    save_parquet_analysis_result(
        cellMetadata.loc[:, ["center_x", "center_y"]].astype(np.float32),
        "cells_centers",
        outputDir,
        index=False,
    )
    save_parquet_analysis_result(
        cellMetadata.loc[:, ["volume"]].astype(np.float32),
        "cells_volume",
        outputDir,
        index=False,
    )
...
def _pack_lod(
    cellsTiledBtrList: list[bytearray], outputDir: Union[str, os.PathLike]
) -> None:
...
def pack_cells(
    ...
    cellGenerator.set_data(
        polyList, texSize[0], texSize[1], transformMatrix, allIndices
    )
